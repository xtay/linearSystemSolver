...
int bicgstab_solver(cMatrix *pA, gVector *pb, gVector *px)
{
    ...
    xArray = (double *)calloc(length, sizeof(double));

    //spaces allocated for these arrays needs to be freed
    rArray = (double *)calloc(length, sizeof(double));
    vArray = (double *)calloc(length, sizeof(double));
        //these two vectors' space will be allocated by duplication later
        //rcArray = (double *)calloc(length, sizeof(double));
        //p_Array = (double *)calloc(length, sizeof(double));
    sArray = (double *)calloc(length, sizeof(double));
    tArray = (double *)calloc(length, sizeof(double));
    tmpArray = (double *)calloc(length, sizeof(double));
    ...
    matrix_vector_product(pA, px, &tmpVec, OLD_SPACE);
    ...
    while(residualNorm > PRECISION && i < 6){
        ...
        matrix_vector_product(pA, &p_Vec, &vVec, OLD_SPACE);
        ...
        matrix_vector_product(pA, &sVec, &tVec, OLD_SPACE);
